/* ══════════════════════════════════════════════════════════════════
   ...
   v29 changes:
   • ROOT CAUSE FIX — "chart shows only one data point": the old code
     combined includeHistory=true AND includeEbay=true in one request.
     Upstream silently falls back to a single "today" snapshot when both
     are requested together. v29 fires two parallel requests and merges:
     one for raw condition history, one for eBay graded sales history.
   • History window capped at 182 days (6 months) — matches upstream
     API's actual retention limit. Requesting 365 was the silent failure.
   • Compare UI rebuilt: condition chips replaced with flat multi-select
     series chips (Raw, PSA 10, PSA 9, BGS BL 10, BGS 10, TAG 10, CGC 10).
     Tap one to plot it; tap multiple to compare them simultaneously.
     No more "Compare All" button — just tap what you want.
   • Portfolio chart badge fix: "X days of history" now queries the actual
     earliest row in price_history instead of last_value_updated, which was
     always "just now" and caused the badge to perpetually show ~0d.
   • Portfolio chart window now anchors to earliest price_history row
     instead of account creation date.
   • Card artwork enlarged (max 520px wide, 720px tall).
══════════════════════════════════════════════════════════════════ */
...
// v29 FIX: this used to derive "days of history" from portfolio_items.last_value_updated,
// but refreshPortfolioValues() sets last_value_updated = now on EVERY refresh for EVERY
// item — so the "oldest" timestamp was always "just now", and the badge always showed 0d.
// Instead we look at the actual earliest row in price_history (the table that accumulates
// daily snapshots over time) and never resets.
let _pfEarliestHistoryDate = null; // cached Date, null = not yet looked up

async function updateHistoryDaysDisplay() {
  const el = document.getElementById('history-days-display');
  if (!el || !_user) return;

  try {
    if (!_pfEarliestHistoryDate) {
      const { data, error } = await _sb
        .from('price_history')
        ...
        .limit(1)
        .maybeSingle();
      if (!error && data?.recorded_date) _pfEarliestHistoryDate = new Date(data.recorded_date);
    }
  } catch (_) { /* non-fatal — fall through to empty state below */ }

  if (_pfEarliestHistoryDate) {
    const today = new Date();
    const totalDays = Math.max(0, Math.round((today.getTime() - _pfEarliestHistoryDate.getTime()) / 86400000));
...
async function loadPortfolioChart() {
  ...
  try {
    ...
    // v29 FIX: use the actual earliest price_history row as the window
    // start instead of (account creation - 10 days). The old approach
    // anchored wrong when price history started well after account creation
    // (e.g. card added months later), or when the user had more data than
    // the 10-day head-start implied.
    const { data: earliestRow } = await _sb
      .from('price_history')
      .select('recorded_date')
      .eq('user_id', _user.id)
      .order('recorded_date', { ascending: true })
    ...
    if (!earliestRow?.recorded_date) throw new Error('No history data');

    const windowStart = new Date(earliestRow.recorded_date);
    windowStart.setDate(windowStart.getDate() - 1); // one day of lead-in padding
...
/* ── Card detail modal — chart + grade selection (v29 rewrite) ──────
   ...
   v29 changes:
   • ROOT CAUSE FIX for "history only shows one point": the old code asked
     upstream for includeHistory=true AND includeEbay=true in a single
     request. Upstream does not reliably return full priceHistory.conditions
     when eBay grading data is also requested in the same call — it falls
     back to a single "today" snapshot, which is exactly the bug reported.
     v29 now fires TWO separate requests in parallel (one for raw condition
     history, one for graded eBay sales history) and merges the results.
   • History window capped at 182 days (6 months) everywhere — matches the
     upstream API's actual retention limit (see api/pokeprice.js).
   • Unified series model: every selectable line (Raw, PSA 10, PSA 9,
     BGS BL 10, BGS 10, TAG 10, CGC 10) lives in one _cdAllSeries map, and
     the user can toggle any combination on/off — true multi-select compare,
     not just a single dropdown + "compare everything" button.
   • Only ONE raw line ("Raw", from the Near Mint condition) is plotted —
     we don't need Lightly Played / Moderately Played / etc. cluttering
     the compare chart.
══════════════════════════════════════════════════════════════════ */
...
let _cdItem            = null;
let _cdAllSeries       = {};        // { seriesKey: [{date, priceSGD}] }
let _cdSelectedKeys    = new Set(); // currently plotted series keys

// Called by openCardDetailModal in index.html to reset state
window._resetCdmGrade = function () {
  _cdApiResult   = null;
  _cdItem        = null;
  _cdAllSeries   = {};
  _cdSelectedKeys = new Set();
};

// The grades users care about, in display order. CGC 10 added in v29.
const SERIES_META = {
...
const SERIES_ORDER = ['raw','psa10','psa9','bgsBL10','bgs10','tag10','cgc10'];
...
function gradeKey(conditionOrGrade) {
  ...
  if (/CGC\s*10/i.test(s)) return 'cgc10';
  ...
  return null; // not one of our tracked series
}

function gradeHistory(salesByGrade, key) {
  // Accept either our internal key (psa10) or the raw upstream key the
  // ebay.salesByGrade object actually uses (varies: "PSA 10", "psa10", etc).
  const entry = salesByGrade?.[key] ?? Object.entries(salesByGrade || {})
    .find(([k]) => gradeKey(k) === key)?.[1];
  if (!entry) return [];
...
// Parses the documented includeHistory response shape:
//   priceHistory.conditions[conditionName][date] = { average, sevenDayAverage, … }
function parseConditionHistory(priceHistory, condName, fxRate) {
  if (!priceHistory) return [];
  const condMap = priceHistory.conditions?.[condName] || null;
  if (!condMap) return [];
  return Object.entries(condMap)
    .map(([date, day]) => ({
      date: date.split('T')[0],
      priceSGD: Math.round(Number(day.sevenDayAverage ?? day.average ?? 0) * fxRate * 100) / 100,
    }))
    .filter(h => h.date && h.priceSGD > 0)
    .sort((a, b) => a.date.localeCompare(b.date));
}
...
async function loadCardDetailChartV28(item) {
  ...
  try {
    ...
    const baseParams = { language: lang, days: '182' };
    const nameSet = { name: item.name || item.set_name || '' };
    if (!isSealed && (item.set_name || item.set)) nameSet.set = item.set_name || item.set || '';

    // v29 FIX: two independent requests instead of one combined call —
    // this is what makes real (multi-day) history come back instead of a
    // single fallback "today" point.
    const rawParams = new URLSearchParams({
      action: isSealed ? 'sealed' : 'search',
      includeHistory: 'true',
      ...baseParams, ...nameSet,
    });
    const gradedParams = new URLSearchParams({
      action: isSealed ? 'sealed' : 'search',
      includeEbay: 'true',
      ...baseParams, ...nameSet,
    });

    const [rawRes, gradedRes] = await Promise.all([
      fetch('/api/pokeprice?' + rawParams).then(r => r.ok ? r.json() : { results: [] }).catch(() => ({ results: [] })),
      isSealed ? Promise.resolve({ results: [] }) : fetch('/api/pokeprice?' + gradedParams).then(r => r.ok ? r.json() : { results: [] }).catch(() => ({ results: [] })),
    ]);

    const rawResults    = rawRes.results    || [];
    const gradedResults = gradedRes.results || [];
    if (!rawResults.length && !gradedResults.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, gradeRowEl); return; }

    const matchOf = (results) => results.find(x => String(x.tcgPlayerId || x.id || x.productId) === String(item.item_id)) || results[0] || null;
    const rRaw    = matchOf(rawResults);
    const rGraded = matchOf(gradedResults);
    _cdApiResult  = rGraded || rRaw;

    // ── Raw series ── only "Near Mint" (the single representative raw line)
    const priceHistObj = rRaw?.priceHistory || rawRes.priceHistory || null;
    let rawSeries = parseConditionHistory(priceHistObj, 'Near Mint', fxRate);
    if (!rawSeries.length && priceHistObj?.conditions) {
      // fall back to whichever condition actually has data
      const firstCond = Object.keys(priceHistObj.conditions)[0];
      if (firstCond) rawSeries = parseConditionHistory(priceHistObj, firstCond, fxRate);
    }
    if (!rawSeries.length) {
      // legacy flat history array (sealed / older cards)
      rawSeries = (rRaw?.history || [])
        .map(h => ({ date: (h.date || '').split('T')[0], priceSGD: Math.round(Number(h.price ?? h.value ?? 0) * fxRate * 100) / 100 }))
        .filter(h => h.date && h.priceSGD > 0)
        .sort((a, b) => a.date.localeCompare(b.date));
    }
    if (!rawSeries.length) {
      const spot = extractPrice(rRaw || rGraded || {});
      if (spot != null) rawSeries = [{ date: new Date().toISOString().split('T')[0], priceSGD: Math.round(spot * fxRate * 100) / 100 }];
    }
    if (rawSeries.length) _cdAllSeries.raw = rawSeries;

    // ── Graded series ──
    const salesByGrade = rGraded?.ebay?.salesByGrade || {};
    for (const key of SERIES_ORDER) {
      if (key === 'raw') continue;
      const hist = gradeHistory(salesByGrade, key);
      if (hist.length) _cdAllSeries[key] = hist;
    }

    const availableKeys = SERIES_ORDER.filter(k => _cdAllSeries[k]?.length);
    if (!availableKeys.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, gradeRowEl); return; }

    // Default selection: the condition/grade that matches this portfolio item, else Raw.
    const myKey = gradeKey(item.condition_or_grade || item.conditionOrGrade || '');
    _cdSelectedKeys = new Set([ (myKey && availableKeys.includes(myKey)) ? myKey : (availableKeys.includes('raw') ? 'raw' : availableKeys[0]) ]);

    _renderCdmSeriesPicker(gradeRowEl, availableKeys);
    _drawCdmChart();
...
// v29: multi-select chip picker — tap any combination of Raw / PSA 10 /
// PSA 9 / BGS BL 10 / BGS 10 / TAG 10 / CGC 10 to compare them together.
function _renderCdmSeriesPicker(gradeRowEl, availableKeys) {
  if (!gradeRowEl) return;
  gradeRowEl.style.display   = 'flex';
  gradeRowEl.style.alignItems = 'center';
  gradeRowEl.style.gap        = '6px';
  gradeRowEl.style.flexWrap   = 'flex-wrap';
  gradeRowEl.style.flexWrap   = 'wrap';

  gradeRowEl.innerHTML = availableKeys.map(key => {
    const meta     = SERIES_META[key];
    const isActive = _cdSelectedKeys.has(key);
    return `<button class="cdm-series-chip" data-key="${key}"
      style="padding:6px 12px;border-radius:20px;border:1px solid ${meta.color}${isActive ? '88' : '33'};background:${isActive ? meta.color + '22' : 'var(--bg3)'};color:${isActive ? meta.color : 'var(--text2)'};font-size:12px;font-weight:600;cursor:pointer;transition:background .15s,color .15s,border-color .15s;"
      onclick="window.toggleCdmSeries('${key}')">${esc(meta.label)}</button>`;
  }).join('');
}

function toggleCdmSeries(key) {
  if (_cdSelectedKeys.has(key)) {
    if (_cdSelectedKeys.size > 1) _cdSelectedKeys.delete(key); // keep at least one line plotted
  } else {
    _cdSelectedKeys.add(key);
  }
  document.querySelectorAll('.cdm-series-chip').forEach(btn => {
    const k        = btn.dataset.key;
    const meta     = SERIES_META[k];
    const isActive = _cdSelectedKeys.has(k);
    btn.style.background  = isActive ? meta.color + '22' : 'var(--bg3)';
    btn.style.color       = isActive ? meta.color        : 'var(--text2)';
    btn.style.borderColor = isActive ? meta.color + '88' : 'rgba(255,255,255,.15)';
  });
  _drawCdmChart();
}
window.toggleCdmSeries = toggleCdmSeries;

function _drawCdmChart() {
  const canvasEl  = document.getElementById('cdm-chart-canvas');
  const loadingEl = document.getElementById('cdm-chart-loading');
  const emptyEl   = document.getElementById('cdm-chart-empty');
  if (!canvasEl) return;
  ...
  // Time range filter (capped to 182 days / 6 months upstream max — see setCdmChartRange)
  ...
  const filterByRange = (arr) => cutoff ? arr.filter(h => new Date(h.date) >= cutoff) : arr;

  const dateSet  = new Set();
  const filtered = {};
  for (const key of _cdSelectedKeys) {
    const pts = filterByRange(_cdAllSeries[key] || []);
    if (pts.length) { filtered[key] = pts; pts.forEach(h => dateSet.add(h.date)); }
  }

  const allDates = [...dateSet].sort();
  if (!allDates.length) { _showCdmEmpty(loadingEl, canvasEl, emptyEl, null); return; }

  const datasets = Object.entries(filtered).map(([key, pts]) => {
    const meta    = SERIES_META[key] || { label: key, color: '#00e5cc' };
    const histMap = Object.fromEntries(pts.map(h => [h.date, h.priceSGD]));
    return { label: meta.label, values: allDates.map(d => histMap[d] ?? null), color: meta.color };
  });
...
let _cdmChartRangeDays = 182; // default 6M (upstream max retention)
...
function setCdmChartRange(days) {
  ...
  if (!_cdApiResult && !Object.keys(_cdAllSeries).length) return;
  _drawCdmChart();
}
