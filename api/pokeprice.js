...
module.exports = async function handler(req, res) {
  ...
  // v29 FIX: upstream only retains 6 months (~182 days) of price history.
  // Requesting more (we were sending 365) caused upstream to silently
  // truncate/empty the window, which is what produced the "only one data
  // point" / "0 days of history" bug. Cap at 182 days everywhere.
  const MAX_HISTORY_DAYS = 182;
  const historyDays = Math.min(parseInt(days, 10) || 30, MAX_HISTORY_DAYS);
